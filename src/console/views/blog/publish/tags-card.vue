<template>
  <Card>
    ...
    <Row>
      <Col span="18">
        <Select v-model="blogTagSelected" multiple @on-change="handleSelectTag" placeholder="请选择文章标签">
...
export default {
  ...
  methods: {
    async initBlogTagList() {
      const res = await this.$store.dispatch('getTags');
      if (localStorage.blogTags) this.blogTagSelected = JSON.parse(localStorage.blogTags)
    ...
    handleDelClick() {
      this.editTagList = [...this.blogTagList]
    ...
    handleSelectTag() {
      localStorage.blogTags = JSON.stringify(this.blogTagSelected); // 本地存储文章标签列表
    }
  }
}
</script>
